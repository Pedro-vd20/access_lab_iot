...
def check_args(args):
    ...
    if len(args) < NUM_ARGS + 1:
        raise(IndexError('Insufficient arguments. Input path to data file, ' + \
            'path to config file, and whether the file is diagnositc or not'))
    ...
    if not (os.path.isfile(data_path) and os.path.isfile(config_path)):
        raise(FileNotFoundError(f'Could not locate {data_path} or {config_path}')) 
...
def check_attr(data, attr, thresh):
    # collects all values above threshhold and records respective times and dates
    ...
    # check data file
        ...
        for sensor in data:
            ...
            if sensor == 'date_time_position':
                ...
                # check if individual sensors have their own in-built diagnostics
